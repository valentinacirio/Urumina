//questo file serve per dire cosa accade quando utente usa la barra di ricerca la quale e classificata come form
document.addEventListener("submit", e => { //al submit del form parte la funzione
	const form = e.target.closest("#search-review-form");//con e.target.closest risale nel dom (che ha struttura ad albero) cercando il primo elemento che trova con id search_review_form (parte dai nodi piu esterni della struttura ad albero del dom)
	if (form) {
		e.preventDefault(); //disattiva la action del form

        //guardo cosa c'e nella text box (che si chiama keywords)
		const keywords = form.querySelector("input[name='keywords']").value; //prendo i valori nel tag input che ha name=keywords
//ora nell'uri cerca keywords cioe cio che ho scritto nella searchbar, la ricerca possibile grazie all'api search.php che invia la richiesta
		sendRequest("search.php?s="+encodeURIComponent(keywords), data => { //quando ottengo data cioe i dati in json delle recensioni trovate
			//creo l'html per mostrare la lista delle recensioni trovate
			...
			//in page-content innietto search_reviews_html tutto in formato html, non piu json
