...
class Review { 
	...
	function read() {
	    ...
	    ORDER BY prodotti.nome_prodotto";
		//preparo la query
		$stmt = $this->conn->prepare($query); 
		//eseguo la query
		$stmt->execute(); //$stmt conterrà il risultato dell'esecuzione della query (in questo caso un recordset)
	...
	function search($keywords) {
		//cerco i prodotti 
		$query = "SELECT * FROM recensioni JOIN prodotti ON recensioni.prodotto_recensito = prodotti.nome_prodotto
				  WHERE recensioni.titolo LIKE ? OR recensioni.testo LIKE ? OR prodotti.nome_prodotto LIKE ?
				  ORDER BY recensioni.titolo;";
		//preparo la query
		$stmt = $this->conn->prepare($query); 
		//% prima e dopo le keywords per estrarre i testi che contengono le keywords
		$keywords = "%{$keywords}%"; 
